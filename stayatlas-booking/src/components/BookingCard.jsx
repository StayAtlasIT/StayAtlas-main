...
// Helper function to parse numbers from decimal objects
...
// Custom Input Component for Check-in and Check-out
...
const BookingCard = ({ property }) => {
      ...
      async function fetchDateAvailability() {
        try {
          const response = await axios.get(
            `/v1/bookings/check-availability/${id}?checkIn=${startDate.toISOString()}&checkOut=${endDate.toISOString()}`
          );
  ...
  const handleApply = async () => {
    ...
    try {
      setError("");
      const { data } = await axios.post("/v1/offers/validate", {
        code: couponCode,
      });
  ...
  const handleBooking = async () => {
    ...
    try {
      const bookingData = {
        ...
        totalAmount: Math.round(charges.totalCost * 100),
      };
      ...
      <div className="lg:sticky lg:top-24">
        <div className="space-y-6 p-6 shadow-xl rounded-2xl text-sm w-full border border-gray-200 bg-gradient-to-br from-green-50 via-yellow-50 to-white ring-1 ring-green-100 transition-all duration-300 ease-in-out">
          ...
          <div className="text-[22px] font-extrabold text-gray-900">
            ...
            {(() => {
              ...
              if (startDate && endDate && totalNights > 0) {
                ...
                const isWeekend = day === 5 || day === 0 || day === 6;
                return isWeekend
                  ? parseDecimal(
                      property?.pricePerNightBoth?.weekend || 0
                    ).toFixed(0)
                  : parseDecimal(
                      property?.pricePerNightBoth?.weekday || 0
                    ).toFixed(0);
              }
            })()}
              ...
              <div className="w-full sm:w-1/2 border-r sm:pr-4">
                <DatePicker
                  ...
                  onChange={(date) => {
                    ...
                    // Automatically open checkout calendar
                    if (checkoutRef.current) {
                      checkoutRef.current.setOpen(true); // opens the checkout calendar
                    }
                  }}
                  selectsStart
                  startDate={startDate}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-green-600">
                      You saved ₹{couponDiscountAmount.toFixed(2)} on this
                      booking
                    </span>
                  </div>
                </div>
                ...
                <tbody>
                  {charges.weekdayNights > 0 && (
                    <tr>
                      ...
                      <td className="text-right">
                        ₹{charges.weekdayNightsCost.toFixed(2)}
                      </td>
                    </tr>
                  )}
                  {charges.weekendNights > 0 && (
                    <tr>
                      ...
                      <td className="text-right">
                        ₹{charges.weekendNightsCost.toFixed(2)}
                      </td>
                    </tr>
                  )}
                  <tr>
                  ...
                  {appliedCoupon && charges.couponDiscountAmount > 0 && (
                    <tr>
                      ...
                      <td className="text-right text-green-600">
                        -₹{charges.couponDiscountAmount.toFixed(2)}
                      </td>
                    </tr>
                  )}
                  <tr>
                  ...
                  <tr>
                    ...
                    <td className="text-right">
                      +₹{charges.additionalCharges.toFixed(2)}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <button
            ...
            } transition text-white font-bold w-full py-2 rounded-xl shadow-md`}
          >
            {isLoading ? (
