...
// Handle search parameters from URL
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        console.log("🔍 Raw Villas Data:", res.data);
        setVillas(res.data || []);
      })
      .catch((err) => {
  ...
  }, [locationParam, startDate, endDate, guests, rooms]);
...
<div className="flex items-center text-blue-600 text-sm ml-4 sm:ml-10 mt-4">
  <Link to="/" className="hover:text-blue-500">Home</Link>
  {locationParam ? (() => {
    const parts = locationParam.split(" - ");
    const villa = parts[0].trim();
      ...
      <>
        {cityState && (
          <>
            ...
            <span className="font-medium text-gray-800">{cityState}</span>
          </>
        )}
        {villa && (
          <>
            <span className="mx-2">{'>'}</span>
            <span className="font-medium text-gray-800">{villa}</span>
          </>
        )}
      </>
    );
...
function VillaCard({ villa, nights = 1, isCompact = false }) {
...
const handleLike = async () => {
    // ✅ Use isLoggedIn state for the check
    if (!isLoggedIn) {
    ...
    setLoadingLike(true);
    try {
      // ✅ Removed withCredentials: true as it's set globally
  ...
  return isCompact ? (
    ...
    <div className="flex gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-medium text-gray-900 truncate">{villa.name}</h3>
        <p className="text-sm text-gray-500 truncate">{villa.location}</p>
      ...
      <div className="flex-1 md:w-3/5 p-3 sm:p-4 md:p-6 flex flex-col justify-between min-h-full">
        ...
        <div className="hidden md:flex mt-4 md:mt-6 flex-row justify-between items-center gap-3 md:gap-4">
          ...
          <button
            ...
            onClick={() => window.location.href = `/booking/${safeVilla.id}`}
          >
            View →
          </button>
        </div>
